        ...
        <tr class="font-bold text-left">
          ...
          <th v-if="url" class="pl-2">
            <span class="flex"> Үйлдэл
              <div v-if="insertUrl" class="col-sm-2 ml-2">
                <inertia-link :href="insertUrl"
        ...
        <tr v-for="(row, index) in datas.data" :key="row.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="pl-2 py-2 border-t">
            <div v-if="url" class="row flex">
              <div class="col-sm-2">
                <inertia-link v-if="url" :href="route(url, row.id)"
              ...
              <div v-if="deleteUrl" class="col-sm-2">
                <inertia-link v-if="url" @click="() => destroy(row.id)" as="buttom"
              ...
              <div v-if="insertUrl" class="col-sm-2">
                <inertia-link v-if="url" :href="insertUrl"
                  class="flex px-2 mr-4 bg-green-500 p-2 text-white rounded-md hover:bg-green-600">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-5 h-5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
              ...
              <div v-if="showUrl" class="col-sm-2">
                <inertia-link v-if="url" :href="route(showUrl, row.id)"
                  class="flex px-2 mr-4 bg-orange-500 p-2 text-white rounded-md hover:bg-orange-600">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-5 h-5">
                    <path stroke-linecap="round" stroke-linejoin="round"
