<template>
  <div class="flex justify-between px-4 mt-4 sm:px-8">
    <h2 class="text-xl text-gray-600">
      <inertia-link class="text-gray-600 hover:text-gray-800 font-bold flex" :href="route('admin.reasons.index')">
    ...
    <div class="flex items-center space-x-1 text-xs">
      ...
      <span class="text-gray-600 text-sm">Шалтгаан засах</span>
    </div>
  </div>
      ...
      <div class="bg-white rounded shadow w-2/5">
        <form @submit.prevent="submit">
          <div class="p-8 -mr-6 -mb-8 flex-wrap">
            <MyInput v-model="form.name" type="text" :error="errors.name" label="Шалтгаан" />
            <MySelect :value="data.place" :error="errors.place_id" class="py-3" label="Байгууллага"
              :url="`/admin/places`" @changeId="(id) => (form.place_id = id)" />
          </div>
