        ...
        <form @submit.prevent="submit">
          <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
            <MyInput v-model="form.name" type="text" :error="errors.name" class="pr-6 pb-8 w-full lg:w-1/2"
              label="Шалтгаан" />
            <MySelect :value="null" type="text" :error="errors.place_id" class="pr-6 pb-8 w-full lg:w-1/2"
              label="Байгууллага" :url="`/admin/places`" @changeId="(id) => (form.place_id = id)" />
          </div>
          <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex justify-end items-center">
            <loading-button :loading="form.processing" class="btn-indigo" type="submit">Хадгалах</loading-button>
          </div>
        </form>
      </div>
    </div>
