      ...
      <div class="w-full">
        <ul class="flex flex-row flex-wrap pt-3 pb-4 mb-0 list-none">
          <li class="flex-auto mr-2 -mb-px text-center last:mr-0">
            <a class="block px-5 py-3 text-xs font-bold leading-normal text-orange-600 uppercase bg-white rounded shadow-lg" @click="form.status_id = 2" :class="{ '!text-white !bg-orange-600': openTab === 2 }">
              Илгээсэн
            </a>
          </li>

          <li class="flex-auto mr-2 -mb-px text-center last:mr-0">
            <a class="block px-5 py-3 text-xs font-bold leading-normal text-green-600 uppercase bg-white rounded shadow-lg" @click="form.status_id = 3" v-bind:class="{ '!text-white !bg-green-600': openTab === 4 }">
              Шийдвэрлэсэн
            </a>
          </li>
                  ...
                  <div class="flex justify-between">
                    <div>
                      <div class="relative text-gray-400">
                        ...
                        <input id="search" name="search" type="search" v-model="form.search" class="w-full py-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-md focus:outline-none focus:ring-gray-500 focus:z-10" placeholder="Бүртгэл хайх" />
                      </div>
                    </div>
                  ...
                  <table class="w-full mt-2 text-gray-500">
                    <thead class="border-b">
                      <tr>
                        <th class="p-2 text-left text-gray-600">
                          <input type="checkbox" class="w-5 h-5 text-blue-500 border-gray-300 rounded cursor-pointer focus:ring-0" />
                        </th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                      <tr v-for="data in datas.data">
                        ...
                        <td>
                          <span v-if="data.status_id == 2" class="px-2 py-1 text-xs text-white bg-orange-500 rounded">
                            {{ data.status?.name ?? 'хоосон' }}
                          </span>
                          <span v-if="data.status_id == 3" class="px-2 py-1 text-xs text-white bg-green-500 rounded">
                            {{ data.status?.name ?? 'хоосон' }}
                          </span>
                          <span v-if="data.status_id == 4" class="px-2 py-1 text-xs text-white bg-blue-500 rounded">
                            {{ data.status?.name ?? 'хоосон' }}
                          </span>
                        </td>
                        ...
                        <td class="text-right">
                          <Menu as="div" class="relative inline-block text-left">
                            <div>
                              <MenuButton class="inline-flex justify-center w-full px-4 py-2 text-sm font-medium text-white bg-gray-500 rounded-md hover:bg-gray-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 ">
                                Үйлдэл
                            ...
                            <Transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
                              <MenuItems class="absolute right-0 z-50 w-32 mt-1 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                <div class="px-1 py-1">
                                  ...
                                  <MenuItem v-if="data.status_id == 3 || ['mha'].includes(auth.user.roles) || ['da'].includes(auth.user.roles) || ['admin'].includes(auth.user.roles)" v-slot="{ active }">
