    ...
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          <my-select :value="null" type="text" :error="errors.register_id" class="pr-6 pb-8 w-full lg:w-1/2" label="Register Id" :url="`${host}/commons?t=register_id`" @changeId="id=>form.register_id=id" />
        ...
        <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex justify-end items-center">
          <loading-button :loading="form.processing" class="btn-indigo" type="submit">Create Register Histories</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  metaInfo: { title: 'Create Register Histories' },
  components: {
    LoadingButton,
    NumberInput,
    MySelect,
  ...
  methods: {
    submit() {
      this.form.post(this.route('admin.register_histories.store'))
    },
  },
}
</script>
