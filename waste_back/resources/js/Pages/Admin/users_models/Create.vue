<template>
  <div class="flex justify-between px-4 mt-4 sm:px-8">
    ...
    <div class="flex items-center space-x-1 text-xs">
      ...
      <span class="text-sm text-gray-600">Хэрэглэгч бүртгэл</span>
    </div>
  </div>
      ...
      <div class="w-2/5 bg-white rounded shadow pb-60">
        <form @submit.prevent="submit">
          <div class="flex-wrap p-8 -mb-8 -mr-6">
            ...
            <MyInput v-model="form.password_confirmation" type="password" autocomplete="new-password" :error="errors.password_confirmation" label="Нууц үг баталгаажуулалт" />

            <MySelect :value="aimag_city" :error="errors.aimag_city_id" :disabled="auth.user.roles == 'da' || auth.user.roles == 'mha'" label="Аймаг/нийслэл" :url="`/admin/aimag_cities`" @changeId="
              (id) => {
            ...
            <MySelect v-if="form.aimag_city_id" :value="soum_district" :disabled="auth.user.roles == 'da' || auth.user.roles == 'mha'" :error="errors.soum_district_id" label="Сум/дүүрэг" :url="`/admin/soum_districts?aimag_city_id=${form.aimag_city_id}`" @changeId="
              (id) => ((form.soum_district_id = id), (form.bag_horoo_id = null))
            " />

            <MySelect v-if="form.soum_district_id" :value="null" :error="errors.bag_horoo_id" label="Баг/хороо" :url="`/admin/bag_horoos?soum_district_id=${form.soum_district_id}`" @changeId="(id) => (form.bag_horoo_id = id)" />
            <CustomSelect v-model="form.position" class="mb-0" clearable required valueKey="name" :options="positions" label="Албан тушаал" />
