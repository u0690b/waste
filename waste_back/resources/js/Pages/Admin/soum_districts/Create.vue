<template>
  <div class="flex justify-between px-4 mt-4 sm:px-8">
    <h2 class="text-xl text-gray-600">
      <ILink class="text-gray-600 hover:text-gray-800 font-bold flex" :href="route('admin.soum_districts.index')">
    ...
    <div class="flex items-center space-x-1 text-xs">
      ...
      <span class="text-gray-600 text-sm">Сум/дүүрэг бүртгэл</span>
    </div>
  </div>
      ...
      <div class="bg-white rounded shadow w-2/5">
        <form @submit.prevent="submit">
          <div class="p-8 -mr-6 -mb-8 flex-wrap">
            ...
            <MySelect :value="null" :error="errors.aimag_city_id" class="my-3" label="Аймаг/нийслэл нэр" :url="`/admin/aimag_cities`" @changeId="(id) => (form.aimag_city_id = id)" />
          </div>
          <div class="flex justify-center">
            <button :loading="form.processing" class="flex bg-gray-600 p-3 mx-4 my-3 text-white rounded text-base hover:bg-gray-500" type="submit">
              Хадгалах
            </button>
            <button :loading="form.processing" class="flex bg-gray-600  p-3 mx-4 my-3 text-white rounded text-base hover:bg-gray-500">
              <ILink class="text-white hover:text-white" :href="route('admin.soum_districts.index')">
                Буцах</ILink>
            </button>
          </div>
        </form>
