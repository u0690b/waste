...
import { WasteFormModel, WasteModel } from '@/types/type';
...
onMounted(async () => {
    const data: WasteModel = { ...props.waste! };

    center.value = {
        lat: props.waste?.lat ?? 40.689247,
        lng: props.waste?.long ?? -74.044502,
    };
    data.images = [];
...
const onSubmit = () => {
    ...
    wasteStore.wasteList = wasteStore.wasteList.filter((v) => JSON.stringify(v) !== JSON.stringify(props.waste));
    wasteStore.wasteList.push(form.model);
    console.log('object');
    emit('done',form.model);
};
</script>
