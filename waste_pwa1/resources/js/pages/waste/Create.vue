...
import LegalInput from '@/components/My/LegalInput.vue';
...
const soumDistict = computed(() => {
    return commonStore.commons?.soum_districts.filter((v: any) => v.aimag_city_id == form.model.aimag_city_id);
});

const bagHoroo = computed(() => {
    return commonStore.commons?.bag_horoos.filter((v: any) => v.soum_district_id == form.model.soum_district_id);
});
        ...
        <div>
            <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div class="relative rounded-md p-1">
                    <GoogleMap api-key="AIzaSyBX2h1XKlleDEXJCKTekPVDk2lI2LNDFNc" style="width: 100%; height: 300px" :center="center" :zoom="15">
                ...
                <form @submit.prevent="onSubmit">
                    <div class="grid gap-3">
                        <USwitch v-model="form.model.whois" size="lg" :label="!form.model.whois ? 'Хуулийн эргээд' : 'Иргэн'" />
                        <UFormField required v-if="!form.model.whois" hint="Зөрчил гаргасан иргэний." label="Овог нэр">
                            <UInput required size="lg" v-model="form.model.name" placeholder="Oвог Нэр" class="w-full" />
                        </UFormField>
                        <UFormField required v-if="!form.model.whois" label="Регистер">
                            <UInput
                                required
                                size="lg"
                                v-model="form.model.register"
                                v-maska="'ZZ########'"
                                placeholder="xx########"
                                data-maska-tokens="{ 'Z': { 'pattern': '[a-zA-Z]|[А-Яа-яӨөҮүЁё]' }}"
                                class="w-full"
                            />
                        </UFormField>

                        <UFormField required v-if="form.model.whois" hint="Зөрчил гаргасан иргэний." label="Албан байгууллага нэр">
                            <LegalInput
                                required
                                size="lg"
                                ...
                                @on-selected="
                                    (item) => {
                                        form.model.register = item?.register;
                                        form.model.chiglel = item?.industry;
                                    }
                                "
                            />
                        </UFormField>
                        <UFormField required v-if="form.model.whois" label="Албан байгууллага регистер">
                            <UInput required size="lg" v-model="form.model.register" placeholder="Регистер" class="w-full" />
                        </UFormField>

                        <UFormField required v-if="form.model.whois" label="Үйл ажиллагааний чиглэл">
                            <UInputMenu
                                openOnClick
                                v-model="form.model.chiglel"
                                valueKey="name"
                                labelKey="name"
                                :items="commonStore.commons?.industry"
                                placeholder="Үйл ажиллагааний чиглэл"
                                class="w-full"
                        ...
                        <UFormField required label="Аймаг нийслэл">
                            <UInputMenu
                                ...
                                required
                                size="lg"
                                v-model="form.model.aimag_city_id"
                                valueKey="id"
                                labelKey="name"
                                :loading="commonStore.loading"
                                :items="commonStore.commons?.aimag_cities"
                        ...
                        <UFormField v-if="form.model.aimag_city_id == undefined || form.model.aimag_city_id < 23" required label="Сум дүүрэг">
                            <UInputMenu
                                openOnClick
                                required
                                size="lg"
                                v-model="form.model.soum_district_id"
                                valueKey="id"
                                labelKey="name"
                                :loading="commonStore.loading"
                                :items="soumDistict"
                        ...
                        <UFormField v-if="form.model.aimag_city_id == undefined || form.model.aimag_city_id < 23" required label="Баг хороо">
                            <UInputMenu
                                openOnClick
                                required
                                size="lg"
                                v-model="form.model.bag_horoo_id"
                                valueKey="id"
                                labelKey="name"
                                :loading="commonStore.loading"
                                :items="bagHoroo"
                        ...
                        <UFormField required label="Хаяг тоот, утас">
                            <UInput required size="lg" v-model="form.model.address" placeholder="Хаяг тоот, утас" class="w-full" />
                        </UFormField>
                        <UFormField required label="Зөрчлийн төрөл">
                            <UInputMenu
                                ...
                                required
                                size="lg"
                                v-model="form.model.reason_id"
                                valueKey="id"
                                labelKey="name"
                                :loading="commonStore.loading"
                                :items="commonStore.commons?.reasons"
                                placeholder="Сум дүүрэг"
                                class="w-full"
                                :search-input="{ icon: 'i-lucide-search' }"
                            >
                                <template #item="{ item }">
                                    <div v-if="item" class="w-full hover:bg-gray-200">
                                        <div class="text-xs text-muted">{{ item.sub_group }}</div>
                        ...
                        <UFormField required label="Гаргасан зөрчилийн байдал">
                            <UTextarea
                                ...
                                placeholder="Гаргасан зөрчилийн байдал, тайлбар"
                                class="w-full"
                            />
                        </UFormField>
